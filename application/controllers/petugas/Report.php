...
class Report extends CI_Controller {
        ...
        public function pengembalian()
    {
        ...
        if(!empty($cek) && $stts=='petugas')
        {
            ...
            $data['data_detail_pinjam'] = $this->Buku_model->getAllData("tb_detail_pinjam","id_detail_pinjam",$id);
            $data['data_buku'] = $this->Buku_model->getAllData("tb_buku");
            ...
            $data['data_buku'] = $this->Buku_model->getAllData("tb_buku");
            //$this->Buku_model->generatePdf($kolom, $data,"Laporan Data Siswa");
            $tmp['content']=$this->load->view('petugas/report/kembali/View_report_kembali',$data, TRUE);
            $this->load->view('petugas/layout',$tmp);
        }
        else
        /*jika status login NO atau status bukan admin kembalikan ke login*/
     ...
     public function pdf_kembali()
    {
        ...
        if(!empty($cek) && $stts=='petugas')
        {
            ...
            $data['data_detail_buku'] = $this->Buku_model->getAllData("tb_detail_buku");
            $data['data_buku'] = $this->Buku_model->getAllData("tb_buku");
            $this->load->view('petugas/report/kembali/View_report_kembali_pdf',$data);
        }
        else
        {
            header('location:'.base_url().'web/log');
 ...
 public function excel(){
        ...
        if(count($data_buku)>0)
        {
            ...
            for ($a=0;$a<11; $a++) 
            {
                ...
                $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(25); // NIM
